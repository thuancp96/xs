...
class S7zballController extends Controller
{

    public function loginmobile(Request $request){
        $user = auth()->user();
        if(isset($user["name"])){
            $s_7zballModule = new Soccer7zballHelpers;
    ...
    public function info(Request $request){
        ...
        if(isset($user["name"])){
            Soccer7zballHelpers::CreateMember($user["name"]); // tao tai khoan truoc khi login
            ...
            //     ->where('game_id',4001)
            //     ->where('created_user',$user->id)->first();
            return response()->json(['code'=>200,'message'=>'','mainbalance' => $user["remain"] ,'SABAbalance' => Soccer7zballHelpers::CheckUsrBalance($user["name"]), 'MaxTransfer' => (int)$user["remain"], 'MaxBet' => $limitMember->change_max_one ]); //$SabaModule->CheckUsrBalance($user["name"])
    ...
    public function transfer(Request $request){
        $user = auth()->user();
        if(isset($user["name"])){
            $SabaModule = new Soccer7zballHelpers;
