...
class SabaController extends Controller
{
    ...
    public function loginmobile(Request $request){
        $user = auth()->user();
        if(isset($user["name"])){
            $SabaModule = new SabaHelpers;
            return redirect($SabaModule->Login($user["name"],2));
    ...
    public function login(Request $request){
        ...
        if(isset($user["name"])){
            $SabaModule = new SabaHelpers;
            return redirect($SabaModule->Login($user["name"]));
            // return response($SabaModule->Login($user["name"]));
        }else{
            return response()->json(['code'=>200,'message'=>'','data' => "error"]);
        }
    ...
    public function info(Request $request){
        ...
        if(isset($user["name"])){
            ...
            return response()->json(['code'=>200,'message'=>'','mainbalance' => $user["remain"]/1000 ,'SABAbalance' => $user["remain_saba"], 'MaxTransfer' => 10000000/1000, 'MaxBet' => 10000000/100 ]); //$SabaModule->CheckUsrBalance($user["name"])
