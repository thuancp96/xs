...
class GetLiveXoSoMienNam extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'get:getlivexosomiennam';
	 ...
	 * @var string
	 */
	protected $description = 'get live xo so mien nam';
	...
	public function handle()
	{
        // $this->generate();
        // return;
        // NotifyHelpers::SendMailNotification('Start XS Mien Nam');
        $count =0;
        while (!$this->generate())
        {
            echo 'run';
        ...
        // NotifyHelpers::SendMailNotification('Finish XS Mien Nam');
    ...
    public function generateByMinhNgoc(){
        for($i=1;$i<=1;$i++)
            try{
                ...
                $now = date('Y-m-d');
                $kqxs1 = XoSoResult::where('location_id', 21)
                ->where('date', $now)->get();

                $kqxs2 = XoSoResult::where('location_id', 22)
                ->where('date', $now)->get();
                ...
                }else 
                {
                    ...
                    $linkminhngoc = 'https://www.minhngoc.net.vn/xo-so-truc-tiep/mien-nam.html';
                    ...
                    if (!isset($date))
                        return 2;
                    $date = str_replace('TRỰC TIẾP XỔ SỐ Miền Nam - ','',$date);
