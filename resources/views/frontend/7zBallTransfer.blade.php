...
<div id="transfer-modal-7zball" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header">
				<h6 class="modal-title" style="color:white" id="sabaModalLabel">CHUYỂN TIỀN 7zBall</h6>
			</div>
			<form id="change-pass-form" class="form-horizontal" data-parsley-validate novalidate>
				<div class="modal-body">
					...
					<div class="modal-footer d-block  text-center">
						...
						<a id="btnClickSaba" onclick='window.open("http://99lucky.com/7zball", "_blank");' style='display: none'></a>
					</div>
...
<script>
	...
	function thuhoi() {
		...
		$.get("/api/7zball/recall", function(data, status) {
			...
			if (data.message != true) {
				...
				$("#transfer-modal-7zball").modal("hide");
				Swal.fire("", data.message, "error").then((value) => {
					if (!isloaded) {
						...
						if (isMobile.any) {
							...
							$('.container.white-bg').show();
							$('.loadere').hide();
							$.get("/api/7zball/info", function(data, status) {
							...
							//   $("#transfer-modal-7zball").modal("show");
						}
					}
				});;
			} else {
				if (window.location.href.indexOf("thongtintk") == -1) {
					isMobile.any ? true : document.getElementById('MainSabaGame').src = "/api/7zball";
					$('.container.white-bg').hide();
					$('.loadere').show();
				} else {
					$('.container.white-bg').show();
	...
	function Chuyen(mode = "IN") {
		...
		$.get("/api/7zball/transfer?mode=" + mode + "&money=" + Number($('#moneyTranssaba').val().replaceAll(',', '')), function(data, status) {
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		try {
			if (window.location.href.indexOf("thongtintk") != -1) {
				$.get("/api/7zball/info", function(data, status) {
					// $("#transfer-modal-7zball").modal("show");
					$("#total_money_bbin").html(data.SABAbalance.toLocaleString('en-US'));
					$(".card-text.mainbalancesaba").html(data.mainbalance.toLocaleString('en-US'));
					$(".card-text.gamebalancesaba").html(data.SABAbalance.toLocaleString('en-US'));
					$(".card-text.maxtransfersaba").html(data.MaxTransfer.toLocaleString('en-US'));
			...
			$('#moneyTranssaba').focusout('input', function(e) {
				...
				moneyMaxTransfer = Number($(".card-text.maxtransfersaba").html().replaceAll(',', '').replaceAll('.', ''))
				moneyMaxBet = Number($(".card-text.maxbetsaba").html().replaceAll(',', '').replaceAll('.', ''))
				if ($("#sabaModalLabel").html() == "CHUYỂN TIỀN 7zBall" || $("#sabaModalLabel").html() == "CHUYỂN TIỀN") {
					maxxx = moneyMax > moneyMaxTransfer ? moneyMaxTransfer : moneyMax
					if (maxxx > moneyInGame)
						maxxx = maxxx - moneyInGame
					else moneyTarget = 0
	...
	function chuyentienClick() {
		...
		$($('#ruttien')).html('Rút tiền');
		$("#transfer-modal-7zball").modal("show");
		$("#sabaModalLabel").html("CHUYỂN TIỀN 7zBall");
	...
	function ruttienClick() {
		...
		$($('#ruttien')).html('Rút tiền');
		$("#transfer-modal-7zball").modal("show");
		$("#sabaModalLabel").html("RÚT TIỀN TỪ 7zBall");
