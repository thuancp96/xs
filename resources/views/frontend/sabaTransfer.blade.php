...
.mainbalancesaba, .gamebalancesaba, .maxtransfersaba{
    font-weight: bold;
}
...
<div id="transfer-modal-saba" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;" data-backdrop="static" data-keyboard="false" >
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header">
				 <h6 class="modal-title" style="color:white" id="sabaModalLabel">CHUYỂN TIỀN SABA SPORTS</h6>
			</div>
				...
				<div class="modal-body">
    				    ...
    				    <div class="card">
                          <div class="card-body">
                            <div class="c-saba-popup-header-balance">
                                <span class="c-text" title="Số Dư Khả Dụng">Số Dư Khả Dụng (1 SABA = 1000)</span>
                                <div clas="row"><span class="card-text mainbalancesaba"></span> SABA</div>
                            </div>
				    ...
				    <div class="mb-3">
    				    <div class="card">
                          <div class="card-body">
                             <div class="c-saba-popup-header-balance">
                                <span class="c-text" title="Số Dư Khả Dụng">Số Dư SABA (1 saba = 1000)</span>
                          ...
                          <div class="card-body">
                            <div class="c-saba-popup-header-balance">
                                <span class="c-text" id="titlemoneyTranssaba"  title="Số Dư Khả Dụng" style="margin-top: -15;">Số SABA chuyển</span>
                            </div>
...
<script>
    ...
    function Chuyen(mode="IN"){
        ...
        $.get("/api/saba/transfer?mode="+mode+"&money=" +  Number($('#moneyTranssaba').val().replaceAll(',','')), function(data, status){
                ...
                Swal.fire("","Chuyển tiền thành công","success").then((value) => {
				  ...
				  if (window.location.href.indexOf("thongtintk") != -1){
					...
					if (window.location.href.indexOf("saba") != -1){
						isMobile.any ? true : document.getElementById('MainGameSaba').src = "/api/sabagames";
						if(isMobile.any)   {
							$('#btnClickSaba').trigger('click');
							$('.container.white-bg').show(); $('.loadere').hide();
							$("#transfer-modal-saba").modal("show");
    ...
    document.addEventListener('DOMContentLoaded', () => {
		...
		try{
			...
			$this.val(moneyTarget.toLocaleString('en-US'));
			$('#moneyConvertToUser').html( (moneyTarget*1000).toLocaleString('en-US'));
			});
			...
			moneyMaxTransfer = Number($(".card-text.maxtransfersaba").html().replaceAll(',', '').replaceAll('.', ''))
			moneyMaxBet = Number($(".card-text.maxbetsaba").html().replaceAll(',', '').replaceAll('.', ''))
			if ($("#sabaModalLabel").html() == "CHUYỂN TIỀN SABA SPORTS" || $("#sabaModalLabel").html() == "CHUYỂN TIỀN"){
				maxxx = moneyMax > moneyMaxTransfer ? moneyMaxTransfer : moneyMax
				if (maxxx > moneyInGame)
					maxxx = maxxx - moneyInGame
				else moneyTarget = 0
