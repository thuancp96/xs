			...
			<div class="card-box">
				...
				<div class="row">
					<div class="table-rep-plugin">
						<div class="table-responsive" >
                            ...
                            @if ($total_point<0)
												style=" color:red;"
											@endif>{{number_format($all_total_point,0)}}</td>
                            <td class="text_right pr10 suminvoice text-bold"
                            @if ($total_bet_money<0) style=" color:red;" @endif>{{number_format($all_total_bet_money,0)}}</td>
                            <td class="text_right pr10 suminvoice text-bold"
                            @if ($all_total_win_money < 0) style=" color:red;" @endif>{{number_format($all_total_win_money,0)}}</td>
                            </tr>
                        </tfoot>
							</table>
						</div>
				...
				@foreach (GameHelpers::GetAllGame() as $key)
									<?php 
										...
										$location_name='';
									?>
<div id="full-width-modal{{$key->game_code}}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="full-width-modalLabel" aria-hidden="true" style="display: none;"><div class="modal-dialog modal-full"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h4 class="modal-title" id="full-width-modalLabel">Chi tiết cược</h4></div><div class="modal-body"><div class="table-rep-plugin">
<div class="table-responsive">
<table id="table_winlose"  class="table table-bordered mails m-0 table-actions-bar table-striped dataTable no-footer">
    
@if($key->game_code < 3000)
<thead><tr><th>Hội viên</th><th>Đài</th><th>Thể loại</th><th>Thời gian</th><th>Số cược</th><th>Giá</th><th>Trả thưởng</th><th>Điểm</th><th>Thực thu</th><th>Thắng/Thua</th><th>Ghi chú</th></tr></thead><tbody>
@else
<thead><tr><th>Hội viên</th><th>Đài</th><th>Thể loại</th><th>Thời gian</th><th>Mã cược</th><th>Kết quả</th><th>Cược hợp lệ</th><th>Tổng đặt cược</th><th>Thắng/Thua</th></tr></thead><tbody>
@endif
    
@foreach($xosorecords as $xosorecord) @if ($xosorecord->game_id == $key->game_code)
<?php $haveData = true; if ($xosorecord->locationslug>20) $location_name=GameHelpers::ChuyenDoiDaiByDate($xosorecord->locationslug,strtotime($xosorecord->created_at)); else $location_name=$xosorecord->location; $total_bet_money += $xosorecord->total_bet_money;
    if( $xosorecord->game_id > 3000){
        $total_bet_accept_money += $xosorecord->com;
    }
// fix tra thuong
...
@if($xosorecord->game_id < 3000)
			<td class="text_right"> @if ($xosorecord->exchange_rates != 0)
			@if($xosorecord->game_id==29 || $xosorecord->game_id==329 || $xosorecord->game_id==429 || $xosorecord->game_id==529 || $xosorecord->game_id==629 || $xosorecord->game_id==9 || $xosorecord->game_id==309 ||$xosorecord->game_id==409 ||$xosorecord->game_id==509||$xosorecord->game_id==609||$xosorecord->game_id==709 || $xosorecord->game_id==10 || $xosorecord->game_id==310 ||$xosorecord->game_id==410 ||$xosorecord->game_id==510||$xosorecord->game_id==610||$xosorecord->game_id==710 || $xosorecord->game_id==11 || $xosorecord->game_id==311 ||$xosorecord->game_id==411 ||$xosorecord->game_id==511||$xosorecord->game_id==611||$xosorecord->game_id==711
                                            || $xosorecord->game_id==21 || $xosorecord->game_id==20 || $xosorecord->game_id==19)
                                                <?php
                                                    $fact=1;
...
//|| $xosorecord->game_id == 19|| $xosorecord->game_id == 20 || $xosorecord->game_id == 21  || $xosorecord->game_id == 16
	...
	@elseif ($win_money<0) 
	    <td class="text_right text_bold" style=" color:red;">{{number_format($win_money,0)}} 
	@elseif ($win_money==0)
	...
	@endif
	    ...
	    @if($xosorecord->game_id < 3000)
	        <td>{{isset($xosorecord->ipaddr1)?$xosorecord->ipaddr:"" }}</td>
	    @endif
	    
	    </tr> @endif @endforeach</tbody><tfoot><tr>
	        
	        @if($key->game_code < 3000)
	        
    	        <td colspan="7" class="text_right pr10">Tổng cộng</td><td class="text_right pr10 suminvoice">{{number_format($total_point,0)}}</td><td class="text_right pr10 suminvoice">{{number_format($total_bet_money,0)}}</td><td class="text_right pr10 suminvoice" @if ($total_win_money < 0) style=" color:red;" @endif >{{number_format($total_win_money,0)}}</td></tr></tfoot></table></div></div></div><div class="modal-footer"><button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Đóng</button></div></div></div></div>
	        @else
    	        <td colspan="6" class="text_right pr10">Tổng cộng</td><td class="text_right pr10 suminvoice">{{number_format($total_bet_accept_money,0)}}</td><td class="text_right pr10 suminvoice">{{number_format($total_bet_money,0)}}</td><td class="text_right pr10 suminvoice" @if ($total_win_money < 0) style=" color:red;" @endif >{{number_format($total_win_money,0)}}</td></tr></tfoot></table></div></div></div><div class="modal-footer"><button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Đóng</button></div></div></div></div>
	        @endif
...
@endsection
@section('js_call')
	<script src="/assets/admin/js/report.js?v=1.011"></script>
@endsection