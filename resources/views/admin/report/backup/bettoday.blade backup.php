				...
				<div class="row">
					...
					<div class="col-sm-2 col-xs-2">
						<span class="input-group-btn">
							<a style="margin-right:5px;" href="{{url('/rp/bettoday/'.$stDate.'/'.$endDate)}}" class="btn waves-effect waves-light btn-primary" id="btn_view_by_filter">Xem</a>
					...
					<div class="table-responsive" style="overflow-x:hidden">
						<table id="datatable"  class="table table-bordered mails m-0 table-actions-bar table-striped dataTable no-footer table-hover">
							...
							?>	
										...
										// if ($user->roleid == 2)
										// 		$userReport = XoSoRecordHelpers::ReportSpAgCXL($user,$stDate,$endDate);
										// if ($user->roleid == 4)
										// 		$userReport = XoSoRecordHelpers::ReportAgCXL($user,$stDate,$endDate);
										// if ($user->roleid == 5)
										// 		$userReport = XoSoRecordHelpers::ReportTongCXL($user,$stDate,$endDate);
										// if ($user->roleid == 6)
												// $userReport = XoSoRecordHelpers::ReportKhachCXL($user,$stDate,$endDate);
										$type="";
										...
										$userReportTemp = Cache::remember('XoSoRecordHelpers-ReportKhachCXLv220230115'.$user->id.'-'.$stDateTemp.'-'.$endDateTemp.'-'.$type, $cacheTime, function () use ($user,$stDateTemp,$endDateTemp,$type) {
												return  XoSoRecordHelpers::ReportKhachCXLv2($user, $stDateTemp, $endDateTemp, isset($type) ? $type : "all");
											});
										// $userReport = XoSoRecordHelpers::ReportKhachCXLv2($user, $stDate, $endDate, isset($type) ? $type : "all");
										...
										@if($user_current->roleid == 7 && $user_current->id != $user->id)
											...
											@endif
										@else
											<td><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{$user->name}}</a>
											</td>
											<td><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{$user->fullname}}</a></td>
											<td class="text_right text-bold">
												<a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{number_format($userReport[0])}}</a>
											</td>
											<td class="text_right text-bold"
											@if ($userReport[1]<0)
												style=" color:red;"
											@endif
											>
												<a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{number_format($userReport[1])}}</a></td>
											

											<td class="text_right text-bold"><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{number_format(0)}}</a></td>
											@if ($user->roleid == 5)
												<td class="text_right text-bold"
												@if ($userReport[4]<0)
												style=" color:red;"
											@endif
												><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{number_format($userReport[4])}}</a></td>
												<td class="text_right text-bold"
												@if ($userReport[4]<0)
												style=" color:red;"
											@endif
											><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{number_format($userReport[4]+0)}}</a></td>
												<td class="text_right text-bold"
												
											><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" @if (0 - ($userReport[2] + $userReport[4])<0)
												style=" color:red;"
											...
											@else
												...
												style=" color:red;"
											@endif
											><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{number_format($userReport[3])}}</a></td>
												<td class="text_right text-bold"
												@if ($userReport[3]<0)
												style=" color:red;"
											@endif><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="">{{number_format($userReport[3]+0)}}</a></td>
												<td class="text_right text-bold"
												><a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" @if (0 - ($userReport[2] + $userReport[3])<0)
												style=" color:red;"
											@endif>{{number_format(0 - ($userReport[2] + $userReport[3]))}}</a></td>
											@endif
										@endif
										<!-- <td style="text-align: center">
										<a href="{{url('/rp/bettoday-detail/'.$user->id.'/'.$stDate.'/'.$endDate)}}" class="table-action-btn"><i class="md md-assignment"></i></a>
									</td> -->
									</tr>
									@endif
									<?php
