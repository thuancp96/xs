			...
			<div class="portlet"><!-- /primary heading -->
				<div class="portlet-heading">
					<h3 class="portlet-title text-dark text-uppercase">
						Bảng thao tác cược giới hạn cho tài khoản {{Auth::user()->name}}
					</h3>

					<!-- <div class="col-sm-1 ">
						<a class="btn btn-default btn-custom waves-effect waves-light btn-sm @if($locationId == 1) disabled @endif" href="\control-max/mien-bac">Miền Bắc</a>
					</div>
					  
					<div class="col-sm-1" style="margin-left: 5px;">
						<a class="btn btn-default btn-custom waves-effect waves-light btn-sm @if($locationId == 4) disabled @endif" href="\control-max/xs-ao">XS Ảo</a>
					</div> -->
			...
			<div class="card-box">
									...
									>
										...
										<thead>
											...
											<tr>
												...
												@if((Auth::user()->roleid <= 2 && $location->slug != 50 && $location->slug != 60 && $location->slug != 70 && $location->slug != 80 ) || $location->slug == 60 || $location->slug == 70 || $location->slug == 80)
												<th @if($user->roleid == 11 ) hidden @endif @if(Auth::user()->roleid > 2 && $location->slug == 70  && $location->slug == 80) hidden @else @endif>	
												@if($location->slug == 70 || $location->slug == 80) 
												Tối đa thắng
													@else 
													@if($location->slug == 60) 
											...
											<tr >
												...
												<td hidden>@if($user->roleid != 1 )
													...
													<td class="@if($location->slug != 60 && $location->slug != 70 && $location->slug != 80) hidden @endif ">
														...
														data-v-min = "0" {{$games_parent[$count]['game_code']}}
													...
													<td>
														...
														@if($location->slug == 60 || $location->slug == 70 || $location->slug == 80)
														data-parsley-min="{{($game['max_point'])}}" data-parsley-max="1000000000"
															@else
													...
													@if((Auth::user()->roleid <= 2 && $location->slug != 50 && $location->slug != 60 && $location->slug != 70 && $location->slug != 80 ) || $location->slug == 60 || $location->slug == 70 || $location->slug == 80)
													<td @if($user->roleid == 11 ) hidden @endif>
														@if($user->roleid > 2 )
				...
				<div class="row" >
					<div class="col-lg-12" style="text-align: right !important;">
						<button type="button" id="btn_OK" onclick="SaveChangeAllType()" class="btn btn-default btn-custom waves-effect waves-light">Lưu</button> 
					</div>
				</div>
				@else
				<div class="row" >
