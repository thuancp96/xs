...
class Tableify implements TableifyInterface
{
    ...
    public function make()
    {
        ...
        for ($i = 0; $i < count($this->data); $i++) {
            ...
            for ($j = 0; $j < count($this->data[$i]); $j++) {
                ...
                if ($j==0)
                    $line .= $this->su->left($this->data[$i][$j], $field_lengths[$j]) . $this->su->fill($this->seperator_padding) . $this->seperator;// . $this->su->fill($this->seperator_padding);
                else
     ...
     * update a row to get the mb_strlen of
     *
    ...
    protected function calculateRowLength(array $field_lengths, int $seperator_padding, string $seperator)
    {
        ...
        // echo strlen($sample_row) . "\n";
        // $sample_row = rtrim($sample_row);
        
        return mb_strlen($sample_row)-1;
    }
    ...
    protected function maxMultidimensionalArrayStringSize(array $data)
    {
        ...
        for ($i =0; $i < count($data); $i++) {
            for ($j = 0; $j < count($data[$i]); $j++) {
                $temp_length = mb_strlen($data[$i][$j]);
                if (!isset($lengths[$j])) {
